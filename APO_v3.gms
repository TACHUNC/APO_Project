* Course: ADVANCED PROCESS OPTIMIZATION (CENG 70003)
...
* The rest are parameters used for result displacement within GAMS
...
SCALARS
 ...
 a      wighting constant for RED in the objective function[~]          /0.333333333/
 b      weighting constant for Cp in the objective function[~]          /0.333333333/
 c      weighting constant for rho in the objective function[~]         /0.333333333/
* objectives' normalization constants obtained by Pre-Single Objective Optimization
* f^* = (f-f_lb) / (f_ub-f_lb), where f_lb is the lower bound and f_ub is the upper bound.
 RED_lb lower bound for RED     /0.162/
 Cpl_lb lower bound for Cp_l    /51.732/
 rho_lb lower bound for rho     /0.821/
 RED_ub upper bound for RED     /1/
 Cpl_ub upper bound for Cp_l    /82.970/
 rho_ub upper bound for rho     /1.156/
;
...
* Equations needed for the calculation of RED
 ...
 RED_eq         Relative energy difference              (C.M. Hansen)
 RED_ub_eq      Boundary conditions for affinity        (C.M. Hansen & K. Skaarup)
* Equations needed for the calculation of Cp_l
...
* structural constraints for molecule design      
 ester_ub_eq    Number fo ester group in the molecule (Upper Bound)
 ether_ub_eq    Number fo ether group in the molecule (Upper Bound)
 amine_ub_eq    Number fo amine group in the molecule (Upper Bound)
 carbon_ub_eq   Number fo carbon atom in the molecule (Upper Bound)
 carbon_lb_eq   Number fo carbon atom in the molecule (Lower Bound)
;

* Multi-Objective Function, Weighted Sum Method
* sum_i w_i*(f-f_lb)/(f_ub-f_lb)  
MOBJ..      obj     =E= a*(RED-RED_lb)/(RED_ub-RED_lb) + b*(Cp_l-Cpl_lb)/(Cpl_ub-Cpl_lb) + c*(rho-rho_lb)/(rho_ub-rho_lb);
...
* initial guess, Monoethanolamine (MEA)
integercut(IC,i)     = 0;
integercut(IC,'OH')  = 1;
integercut(IC,'CH2') = 1;
